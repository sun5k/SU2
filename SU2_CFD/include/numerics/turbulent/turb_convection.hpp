...
/*!
 * \class CUpwSca_TurbEQ3
 * \brief Class for doing a scalar upwind solver for the 3 equation serises turbulence model equations.
 * \ingroup ConvDiscr
 * \author S. Kang
 */
template <class FlowIndices>
class CUpwSca_TurbEQ3 final : public CUpwScalar<FlowIndices> {
private:
  using Base = CUpwScalar<FlowIndices>;
  using Base::nDim;
  using Base::V_i;
  ...
  /*!
   * \brief EQ3 specific steps in the ComputeResidual method
   * \param[in] config - Definition of the particular problem.
   */
  void FinishResidualCalc(const CConfig* config) override {
    Flux[0] = a0*V_i[idx.Density()]*ScalarVar_i[0] + a1*V_j[idx.Density()]*ScalarVar_j[0];
    ...
    Jacobian_i[2][0] = 0.0;   Jacobian_i[2][1] = 0.0;     Jacobian_i[1][2] = a0;
...
public:
   ...
   * \param[in] config - Definition of the particular problem.
   */
  CUpwSca_TurbEQ3(unsigned short val_nDim, unsigned short val_nVar, const CConfig* config)
    : CUpwScalar<FlowIndices>(val_nDim, val_nVar, config) { bounded_scalar = config->GetBounded_Turb(); }
};