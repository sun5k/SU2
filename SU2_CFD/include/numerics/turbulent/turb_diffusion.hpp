...
template <class FlowIndices>
class CAvgGrad_TurbEQ3_KOG final : public CAvgGrad_Scalar<FlowIndices> {
private:
  using Base = CAvgGrad_Scalar<FlowIndices>;
  using Base::Laminar_Viscosity_i;
  using Base::Laminar_Viscosity_j;
  ...
  void FinishResidualCalc(const CConfig* config) override {
    ...
    Flux[0] = diff_kine*Proj_Mean_GradScalarVar[0];
    Flux[1] = diff_omega*Proj_Mean_GradScalarVar[1];
    Flux[2] = diff_gamma*Proj_Mean_GradScalarVar[2];

    /*--- For Jacobians -> Use of TSL (Thin Shear Layer) approx. to compute derivatives of the gradients ---*/
    if (implicit) {
      const su2double proj_on_rho_i = proj_vector_ij/Density_i;
      Jacobian_i[0][0] = -diff_kine*proj_on_rho_i;  Jacobian_i[0][1] = 0.0;                        Jacobian_i[0][2] = 0.0;
      Jacobian_i[1][0] = 0.0;                       Jacobian_i[1][1] = -diff_omega*proj_on_rho_i;  Jacobian_i[1][2] = 0.0;
      Jacobian_i[2][0] = 0.0;                       Jacobian_i[2][1] = 0.0;                        Jacobian_i[2][2] = -diff_gamma*proj_on_rho_i;

      const su2double proj_on_rho_j = proj_vector_ij/Density_j;
      Jacobian_j[0][0] = diff_kine*proj_on_rho_j;   Jacobian_j[0][1] = 0.0;                       Jacobian_j[0][2] = 0.0;
      Jacobian_j[1][0] = 0.0;                       Jacobian_j[1][1] = diff_omega*proj_on_rho_j;  Jacobian_j[1][2] = 0.0;
      Jacobian_j[2][0] = 0.0;                       Jacobian_j[2][1] = 0.0;                       Jacobian_j[2][2] = -diff_gamma*proj_on_rho_j;
    }
  }
...
public:
   ...
   * \param[in] config - Definition of the particular problem.
   */
  CAvgGrad_TurbEQ3_KOG(unsigned short val_nDim, unsigned short val_nVar,
                   const su2double* constants, bool correct_grad, const CConfig* config)
    : CAvgGrad_Scalar<FlowIndices>(val_nDim, val_nVar, correct_grad, config),
      sigma_k1(constants[0]),
      sigma_k2(constants[1]),
