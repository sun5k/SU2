...
/*!
 * \class CAvgGrad_TransAFMT
 * \brief Class for computing viscous term using average of gradient with correction (AFMT transition model).
 * \ingroup ViscDiscr
 * \author S. Kang.
 */
template <class FlowIndices>
class CAvgGrad_TransAFMT final : public CAvgGrad_Scalar<FlowIndices> {
private:
  using Base = CAvgGrad_Scalar<FlowIndices>;
  using Base::Laminar_Viscosity_i;
  using Base::Laminar_Viscosity_j;
...
public:
   ...
   * \param[in] config - Definition of the particular problem.
   */
  CAvgGrad_TransAFMT(unsigned short val_nDim, unsigned short val_nVar, bool correct_grad, const CConfig* config)
    : CAvgGrad_Scalar<FlowIndices>(val_nDim, val_nVar, correct_grad, config){
  }
 ...
 */
template <class FlowIndices>
class CAvgGrad_TransIntermittency final : public CAvgGrad_Scalar<FlowIndices> {
private:
  using Base = CAvgGrad_Scalar<FlowIndices>;
  using Base::Laminar_Viscosity_i;
  using Base::Laminar_Viscosity_j;
...
public:
   ...
   * \param[in] config - Definition of the particular problem.
   */
  CAvgGrad_TransIntermittency(unsigned short val_nDim, unsigned short val_nVar,
